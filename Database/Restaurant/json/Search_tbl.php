<?php
	...
	//Truy vấn
	$query = mysqli_query($conn, 'SELECT table_id FROM `table` WHERE table_id NOT IN (SELECT or_table FROM `order` WHERE order.or_table = table.table_id AND or_date = '.$date.' AND or_time < '.$hour2.' AND or_time > '.$hour4.')');
