...
?>
	...
	//Truy vấn
	$query = mysqli_query($conn, "SELECT `table_id`,`table_content` FROM `table` WHERE table_id NOT IN (SELECT `or_table` FROM `order` WHERE order.or_table = table.table_id AND or_date = STR_TO_DATE($date2, '%d/%m/%Y') AND or_time < '$mysql1' AND or_time > '$mysql2')");
